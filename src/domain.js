...
export const INITIAL_STATE = {
  ...
  // Real on-chain state (voter weight, locker, delegate, analyst registration) is
  // polled from chain/governance.js and api/governance.js in App.jsx — nothing
  // governance-related is seeded here. `votes` and `delegatedAt` below are the
  // only governance-adjacent local fields left: session-only UI bookkeeping, not
  // a mock ledger (see App.jsx's useDerived for how each is set, always after a
  ...
  chartRange: '30D',
  votingDays: 5,
  // Last time THIS session confirmed an on-chain `set_delegate` call — purely
  // cosmetic (decision #12's client-side switch-cooldown suggestion), never a
  // gate: `set_delegate` itself has no time-lock.
  delegatedAt: null,
  lockAmount: '',
  unlockAmount: '',
  npAction: 'buy',
